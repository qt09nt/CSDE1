...
s2c_CSDE1_Capture <- set_metadata_control_condition(metadata_table=s2c_CSDE1_Capture, reference="Capture")

so_CSDE1_Capture <- sleuth_prep(s2c_CSDE1_Capture, extra_bootstrap_summary = TRUE, transform_fun_tpm = function(x) log2(x + 0.5))

so_CSDE1_Capture <- sleuth_fit(so_CSDE1_Capture, ~condition, 'full')
so_CSDE1_total <-sleuth_fit(so_CSDE1_IGG, ~1, 'reduced')